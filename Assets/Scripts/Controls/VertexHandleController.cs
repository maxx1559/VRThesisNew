...
/*        public OvrAvatarEntity Avatar;*/
        ...
        void Awake()
        {
            ...
            LeftGrabber = GameObject.Find("Grabber_L");
            RightGrabber = GameObject.Find("Grabber_R");
        ...
        void Update()
        {
            ...
            Vector3 left_ctrl_pt = LeftGrabber.transform.position;
            Vector3 right_ctrl_pt = RightGrabber.transform.position;
            ...
            if (activeControllers.Count == 1 && activeControllers[0] == this)
            {
                if (IsDragged)
                {
                    Debug.Log("VertexHandleController: " + "IsDragged is true");
                    ...
                    GrabControl leftGrabControl = LeftGrabber.GetComponent<GrabControl>();
                    GrabControl rightGrabControl = RightGrabber.GetComponent<GrabControl>();

                    //SBS
                    /*if (leftGrabControl.collidedVertexHandle != null && leftGrabControl.HandState.ToString().Equals("TOUCHING") 
                        && !OVRInput.Get(OVRInput.Touch.Any, OVRInput.Controller.LTouch))*/
                    if (leftGrabControl.collidedVertexHandle != null && leftGrabControl.HandState.ToString().Equals("TOUCHING")
                        && !leftGrabControl.GrabState)
                    {
                        Debug.Log("VertexHandleController: " + "Left hand not grabbing, just colliding and touching with vertex handle");
...
*/                        EnableSBSLine(leftGrabControl.collidedVertexHandle.transform.localPosition, translationVector, leftGrabControl.collidedVertexHandle.transform.root.localToWorldMatrix);
                    ...
                    /*else if (rightGrabControl.collidedVertexHandle != null && rightGrabControl.HandState.ToString().Equals("TOUCHING") 
                        && !OVRInput.Get(OVRInput.Touch.Any, OVRInput.Controller.RTouch))*/
                    else if (rightGrabControl.collidedVertexHandle != null && rightGrabControl.HandState.ToString().Equals("TOUCHING") 
                        && !rightGrabControl.GrabState)
                    {
                        Debug.Log("VertexHandleController: " + "Right hand not grabbing, just colliding and touching with vertex handle");
                ...
                if (activeControllers.Count == 2 && refinementActive)
                {
                    if (mode == InteractionMode.DUAL)
                    {
                        ...
                        foreach (VertexHandleController v in activeControllers)
                        {
                            ...
                            if (v.interactingControllerCollider.gameObject.name.EndsWith("_R"))
                            {
                                Debug.Log("VertexHandleController: " + "InteractionMode is DUAL, interacting controller ends with _R");

                                angleDict["right"] = angle;
                            }
                            else if (v.interactingControllerCollider.gameObject.name.EndsWith("_L"))
                            {
                                Debug.Log("VertexHandleController: " + "InteractionMode is DUAL, interacting controller ends with _L");
        ...
        public override void Interact()
        {
            ...
            if (activeControllers.Count == 1)
            {
                ...
                Debug.Log("VertexHandleController: IsDragged is being set to true");
                IsDragged = true;
        ...
        public override void StopInteraction()
        {
            ...
            if (mode == InteractionMode.SINGLE)
            {
                ...
                if (IsDragged)
                {
                    Debug.Log("VertexHandleController: IsDragged is being set to false");
                    IsDragged = false;
            ...
            else
            {
                ...
                if (mode == InteractionMode.DUAL && refinementActive)
                {
                    ...
                    if (activeControllers.Count == 2)
                    {
                        ...
                        Debug.Log("VertexHandleController: IsDragged is set to false for the given activeController");
